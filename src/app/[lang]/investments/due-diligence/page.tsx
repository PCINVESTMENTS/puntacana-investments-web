...
export async function generateMetadata({ params }: { params: Promise<{ lang: 'es' | 'en' }> }): Promise<Metadata> {
    ...
    return {
        title: lang === 'es' ? "Debida Diligencia | Punta Cana Investments" : "Due Diligence | Punta Cana Investments",
        description: lang === 'es'
...
export default async function DueDiligencePage({ params }: { params: Promise<{ lang: 'es' | 'en' }> }) {
    const { lang } = await params;
