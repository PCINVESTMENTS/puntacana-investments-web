...
const client = createClient({
    ...
    token: process.env.SANITY_TOKEN, // Requires a token with write access
    useCdn: false,
});
...
async function migrate() {
    ...
    for (const p of properties) {
        const doc = {
            ...
            featuresEs: p.features.es,
            descriptionEn: p.description.en,
            descriptionEs: p.description.es,
            featured: p.featured,
            coordinates: p.coordinates
            // Images would technically need to be uploaded as assets first, but for now we might skip or use external URL if we change schema to support URL string
            // Sanity 'image' type expects an asset reference. 
            // We will skip image upload in this simple script 
            // OR we imply we need to upload them. 
            // For simplicity, we'll log that images need manual upload or advanced script.
        };

        try {
            const res = await client.create(doc);
            console.log(`Created property: ${p.title} (${res._id})`);
        } catch (err) {
